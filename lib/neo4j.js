...
module.exports = {
  ...
  getDriver(identifier) {
    ...
    if (!_.isUndefined(this.drivers) && _.isUndefined(identifier)) {
    ...
    if (_.isInteger(identifier) && !_.isUndefined(this.drivers)) {
  ...
  close(identifier) {
    ...
    if (!_.isUndefined(this.drivers) && _.isString(identifier)) {
      const unclosed_driver = _.find(this.drivers, ['name', identifier]);
      return unclosed_driver.driver.close();
    }

    if (!_.isUndefined(this.drivers) && _.isInteger(identifier)) {
      ...
      const unclosed_driver = this.drivers[identifier - 1];
      return unclosed_driver.driver.close();
    }
